...
import { selectAuthMethod, type TokenEndpointAuthMethod } from "./tokens.js";
...
export interface OAuthStartRequest {
  ...
  channel: string;
  /** The origin URL (from browser's window.location.origin) - used for redirect_uri */
  origin?: string;
}
...
export interface OAuthApiDependencies {
  ...
  getMcpArtifact: (
    channel: string,
    slug: string
  ) => { props?: { url?: string; auth?: OAuthConfig } } | null;
  ...
  /** Get base URL from request (derives from Host header) */
  getBaseUrl: (req: IncomingMessage) => string;

  /** Generate a deterministic client ID from base URL */
  generateClientId: (baseUrl: string) => string;
}
...
export async function handleOAuthStatus(
  ...
  if (tokens.expiresAt) {
    ...
    if (expiresAt <= new Date()) {
      sendJson(res, 200, { status: "expired", expiresAt: tokens.expiresAt } as OAuthStatusResponse);
      return;
    }
  }
...
export async function handleOAuthStart(
  ...
  const { mcpSlug, channel, origin } = body;
  ...
  const artifact = deps.getMcpArtifact(channel, mcpSlug);
  if (!artifact) {
  ...
  if (authConfig?.type !== "oauth") {
    sendJson(res, 400, { error: "MCP artifact does not have OAuth auth configured" });
    return;
  }

  try {
    // Use origin from body (browser's actual URL) if provided, otherwise fall back to Host header
    const baseUrl = origin || deps.getBaseUrl(req);
    const defaultClientId = deps.generateClientId(baseUrl);
    console.log(`[oauth] Starting OAuth flow with baseUrl: ${baseUrl}`);
...
export async function handleOAuthCallback(
  ...
  try {
    ...
    // Select the best auth method based on server support
    const authMethod = selectAuthMethod(
      pendingState.endpoints.tokenEndpointAuthMethods,
    ...
    // Store tokens with clientId for future refresh
    deps.storeTokens(pendingState.channel, pendingState.mcpSlug, {
...
<body>
  ...
  <noscript>
    ${result.success ? "<p>Authorization successful. You can close this window.</p>" : `<p>Authorization failed: ${result.errorDescription || result.error}</p>`}
  </noscript>
</body>
</html>`;
