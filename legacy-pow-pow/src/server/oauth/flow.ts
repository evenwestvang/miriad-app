...
export interface StartOAuthFlowParams {
  ...
  /** Default client ID to use if dynamic registration fails */
...
export async function startOAuthFlow(
  ...
  const { mcpSlug, channel, mcpUrl, authConfig, baseUrl, defaultClientId } = params;
  ...
  try {
    ...
    console.error(`[oauth] Configure a clientId in the system.mcp artifact's auth config, or ensure the server supports RFC 7591 dynamic registration`);
  ...
  // Store pending state (including clientSecret for confidential clients)
