...
/**
 * Default base directory for cikada data.
 */
const CIKADA_DIR =
  process.env.CIKADA_DIR || path.join(os.homedir(), ".cikada");

/**
 * Directory for OAuth client registrations.
 */
const REGISTRATIONS_DIR = path.join(CIKADA_DIR, "oauth-clients");
...
export function saveRegistration(
  ...
  try {
    fs.writeFileSync(
      registrationPath,
      JSON.stringify(storedRegistration, null, 2),
      {
        mode: 0o600,
      }
    );
...
export async function getOrRegisterClient(
  ...
  if (stored) {
    ...
    if (stored.clientSecretExpiresAt) {
      ...
      if (expiresAt > 0 && Date.now() > expiresAt) {
        ...
        console.log(
          `[oauth] Client secret expired for ${mcpSlug}, re-registering...`
        );
  ...
  // Register new client
  console.log(
    `[oauth] Registering client for ${mcpSlug} at ${metadata.registration_endpoint}`
  );
