 ...
 * This allows the durable driver to use real Claude API calls.
 */

import Anthropic from "@anthropic-ai/sdk";
import type { LLMAdapter, LLMStreamOptions, StreamChunk, ToolDefinition } from "./mock-llm.js";
 ...
 * Implements the same interface as MockLLMAdapter for interoperability.
 */
export class AnthropicLLMAdapter implements LLMAdapter {
  ...
  async *stream(options: LLMStreamOptions): AsyncIterable<StreamChunk> {
    const { model, system, messages, tools, max_tokens } = options;
...
/**
 * Create an Anthropic LLM adapter from an API key.
 */
export function createAnthropicAdapter(apiKey: string): AnthropicLLMAdapter {
