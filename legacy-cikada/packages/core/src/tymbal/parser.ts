 ...
 */
export function startFrame(id: string, metadata?: MessageMetadata): StartFrame {
  if (metadata) {
    return { i: id, m: metadata };
  }
  return { i: id } as StartFrame;
}

/**
 * Create an append frame.
 */
export function appendFrame(id: string, text: string): AppendFrame {
  return { i: id, a: text };
}
 ...
 */
export function resetFrame(id: string): ResetFrame {
  return { i: id, v: null };
}
 ...
 */
export function syncRequest(since?: string): SyncRequest {
  if (since) {
    return { request: 'sync', since };
  }
  return { request: 'sync' };
}
 ...
 */
export function errorFrame(error: string, message: string): ErrorFrame {
